...
class ResearchUser(db.Model):
	__tablename__ = "ResearchUser"
	id = db.Column(db.Integer, primary_key=True)
	
class KeypressGroup(db.Model):
	__tablename__ = "KeypressGroup"
	id = db.Column(db.Integer, primary_key=True)
	keypresses = db.relationship("Keypress")

class Keypress(db.Model):
	__tablename__ = "keypressData"
	user_id = db.Column(db.Integer, db.ForeignKey('ResearchUser.id'))
	group_id = db.Column(db.Integer, db.ForeignKey("KeypressGroup.id"))
	id = db.Column(db.Integer, primary_key=True)
	press_timestamp = db.Column(db.String(15))
	release_timestamp = db.Column(db.String(15))
	keycode = db.Column(db.String(3))

class MouseMoveGroup(db.Model):
	__tablename__ = "MouseMoveGroup"
	id = db.Column(db.Integer, primary_key=True)
	MouseMovees = db.relationship("MouseMove")

class MouseMove(db.Model):
	__tablename__ = "MouseMoveData"
	user_id = db.Column(db.Integer, db.ForeignKey('ResearchUser.id'))
	group_id = db.Column(db.Integer, db.ForeignKey("MouseMoveGroup.id"))
	id = db.Column(db.Integer, primary_key=True)
	timestamp = db.Column(db.String(15))
	x = db.Column(db.String(4))
	y = db.Column(db.String(4))
